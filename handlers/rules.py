...
• 命中规则：
  ...
  - 同时猜中胜平负和比分，两边都参与
• 分配规则：
  - 胜平负命中：先拿回胜平负池本金，再按投入比例分胜平负池剩余
  - 比分命中：先拿回比分池本金，再按比分档位分比分池剩余
  - 比分池没分完的部分，补进胜平负中奖池，按胜平负中奖票投入比例分
• 比分档位：
  - 1 萝卜：最多吃比分剩余池的 5%
  - 10 萝卜：最多吃比分剩余池的 10%
  - 50 萝卜：最多吃比分剩余池的 50%
  - 100 萝卜及以上：最多吃比分剩余池的 100%
• 平台加池：
  - 没有滚存
  - 如果应发超过总奖池，由平台加池补足
• 无人命中：
...
胜平负先拿 50 × 40% = 20，再分胜平负池剩余。
比分先拿 50 × 60% = 30，再最多吃比分剩余池的 50%。
"""
}
