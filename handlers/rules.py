...
• 小奖池保护：
  - 只命中胜平负，最低按本张投入的 1.2 倍发
  - 命中完整比分，最低按本张投入的 1.5 倍发
  - 保底奖励只使用本场因封顶未释放的留存，完整比分优先，整场总发放不超过总奖池
• 用户回流奖励：
  - 平台最多留存 f1bb 补贴部分
  - 结算后如果留存超过补贴，超出的用户下注部分继续发给中奖票
  - 回流按 命中质量 × 投入 分配：完整比分权重 3，胜平负权重 1
• 单票上限：
  - 每张票最终最多发 本张投入 × 10
  - 超过 10 倍的部分不在本场发放，滚存到下一场比赛奖池
• 无人命中：
  - 无人中胜平负，胜平负池留存
  - 无人中比分，比分池留存
• 截止时间：开赛前 10 分钟停止下注。
