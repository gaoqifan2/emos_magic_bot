...
async def robbery_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_success:
        ...
        result_message = (
            f"🎭 *打劫结果*\n"
            f"━━━━━━━━━━━━━━━━━━\n\n"
            f"🦹 打劫者：{robber_username}\n"
            f"🎯 目标：{victim_username}\n"
            f"💰 打劫金额：{amount} 🪙\n\n"
            f"✅ *打劫成功！*\n"
            f"💸 税收：{tax_amount} 🪙（10%）\n"
            f"💵 实际获得：{net_amount} 🪙\n\n"
        ...
        # ========== 打劫失败 ==========
        # 转账（给对方）
        update_balance(robber_emos_id, -amount)
        update_balance(victim_emos_id, amount)
        
        # 记录游戏结果
        from app.database import add_game_record
        add_game_record(robber_emos_id, 'robbery', amount, 'lose', -amount, robber_username)
