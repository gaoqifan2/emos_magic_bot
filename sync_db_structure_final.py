#!/usr/bin/env python3
# 从game_db_test同步数据库结构到game_db（最终版本）
    ...
    try:
        ...
        for table_name in table_names:
            ...
            if not create_table_sql:
                print(f"❌ 无法获取表 {table_name} 的结构，跳过")
                continue
        ...
        if source_conn:
            try:
                source_conn.close()
                print("game_db_test 数据库连接已关闭")
            except:
                pass
        if target_conn:
            try:
                target_conn.close()
                print("game_db 数据库连接已关闭")
            except:
                pass
